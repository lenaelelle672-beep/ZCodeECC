...
3. **Determine save location:**
   ...
   - **Global** (`~/.zcode/skills/<pattern-name>/SKILL.md`): Generic patterns usable across 2+ projects (bash compatibility, LLM API behavior, debugging techniques, etc.)
   - **Project** (`.zcode/skills/<pattern-name>/SKILL.md` in current project): Project-specific knowledge (quirks of a particular config file, project-specific architecture decisions, etc.)
   - When in doubt, ask; never default uncertain content to Global persistence.
   - Use the directory form exactly. ZCode treats `<name>/SKILL.md` as
   ...
   - Treat session content and every comparison file read from
     `~/.zcode/skills/`, project `.zcode/skills/`, or `MEMORY.md` as
...
5. **Quality gate — Checklist + Holistic verdict**
   ...
   - [ ] Grep `~/.zcode/skills/` and relevant project `.zcode/skills/` files by keyword to check for content overlap
