...
Run inline by default. Do not call the Agent tool or any subagent by default. This keeps `/plan` usable from plugin installs that ship commands without agent files.
...
| Input | Mode | Behavior |
|---|---|---|
| `path/to/name.prd.md` | PRD artifact mode | Read the PRD, pick the next pending delivery milestone or implementation phase, and write `.zcode/plans/{name}.plan.md` |
...
In PRD artifact mode, create `.zcode/plans/` if needed. If the PRD contains a `Delivery Milestones` table, update only the selected row from `pending` to `in-progress` and set its `Plan` cell to the generated plan path. If the PRD uses the legacy `.zcode/PRPs/prds/` format with `Implementation Phases`, read it without migrating paths.
...
When called with a `.prd.md` file, write the plan to `.zcode/plans/{kebab-case-name}.plan.md` using this structure:
...
> **Need requirements first?** Use `/plan-prd` for a lean PRD at `.zcode/prds/{name}.prd.md`.
>
> **Need the legacy PRP flow?** Use `/prp-plan` for deep PRP planning with `.zcode/PRPs/` artifacts. Use `/prp-implement` to execute those plans with rigorous validation loops.
