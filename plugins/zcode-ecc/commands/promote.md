...
```bash
python3 "${ZCODE_PLUGIN_ROOT}/skills/continuous-learning-v2/scripts/instinct-cli.py" promote [instinct-id] [--force] [--dry-run]
```

Or if `ZCODE_PLUGIN_ROOT` is not set (manual installation):

```bash
python3 ~/.zcode/skills/continuous-learning-v2/scripts/instinct-cli.py promote [instinct-id] [--force] [--dry-run]
```
...
4. Write promoted instincts to `~/.zcode/homunculus/instincts/personal/` with `scope: global`
