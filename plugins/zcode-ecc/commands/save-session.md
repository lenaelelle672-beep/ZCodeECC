---
description: "Save current session state to a dated file in ~/.zcode/session-data/ so work can be resumed in a future session with full context."
---
...
- End of a work session before closing ZCode
...
```bash
mkdir -p ~/.zcode/session-data
```

### Step 3: Write the session file

Create `~/.zcode/session-data/YYYY-MM-DD-<short-id>-session.tmp`, using today's actual date and a short-id that satisfies the rules enforced by `SESSION_FILENAME_REGEX` in `session-manager.js`:
...
- Use the canonical global session store: `~/.zcode/session-data/`
