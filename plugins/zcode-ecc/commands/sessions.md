---
description: "Manage ZCode session history, aliases, and session metadata."
---

# Sessions Command

Manage ZCode session history - list, load, alias, and edit sessions stored in `~/.zcode/session-data/` with legacy reads from `~/.zcode/sessions/`.
...
```bash
node -e "
const _r = (function(){var p=require('path'),f=require('fs'),o=require('os');var e=process.env.ZCODE_PLUGIN_ROOT;if(e&&e.trim())return e.trim();var d=p.join(o.homedir(),'.zcode');function L(x){try{return require(p.join(x,'scripts','lib','resolve-ecc-root')).resolveEccRoot()}catch(_){return null}}var r=L(d);if(r)return r;var s=['ecc','ecc@ecc','marketplaces/ecc','everything-claude-code','everything-claude-code@everything-claude-code','marketplaces/everything-claude-code'];for(var i=0;i<s.length;i++){r=L(p.join(d,'plugins',s[i]));if(r)return r}try{var g=['ecc','everything-claude-code'];for(var j=0;j<g.length;j++){var c=p.join(d,'plugins','cache',g[j]);var O=f.readdirSync(c);for(var k=0;k<O.length;k++){var q=p.join(c,O[k]);var V=f.readdirSync(q);for(var m=0;m<V.length;m++){r=L(p.join(q,V[m]));if(r)return r}}}}catch(_){}return d})();
...
- Sessions are stored as markdown files in `~/.zcode/session-data/` with legacy reads from `~/.zcode/sessions/`
- Aliases are stored in `~/.zcode/session-aliases.json`
