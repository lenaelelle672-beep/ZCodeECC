...
```bash
python3 "${ZCODE_PLUGIN_ROOT}/skills/continuous-learning-v2/scripts/instinct-cli.py" evolve [--generate]
```

Or if `ZCODE_PLUGIN_ROOT` is not set (manual installation):

```bash
python3 ~/.zcode/skills/continuous-learning-v2/scripts/instinct-cli.py evolve [--generate]
```
...
6. If `--generate` is passed, write files to:
   - Project scope: `~/.zcode/homunculus/projects/<project-id>/evolved/`
   - Global fallback: `~/.zcode/homunculus/evolved/`
