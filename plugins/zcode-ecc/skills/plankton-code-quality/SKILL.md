...
Every time ZCode edits or writes a file, Plankton's `multi_linter.sh` PostToolUse hook runs:
...
# Start ZCode — hooks activate automatically
claude
```

No install command, no plugin config. The hooks in `.zcode/settings.json` are picked up automatically when you run ZCode in the Plankton directory.

### Per-Project Integration

To use Plankton hooks in your own project:

1. Copy `.zcode/hooks/` directory to your project
2. Copy `.zcode/settings.json` hook configuration
...
Plankton's `.zcode/hooks/config.json` controls all behavior:
