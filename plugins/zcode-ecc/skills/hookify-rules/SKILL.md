...
Hookify rules are markdown files with YAML frontmatter that define patterns to watch for and messages to show when those patterns match. Rules are stored in `.zcode/hookify.{rule-name}.local.md` files.
...
### file Events
Match Edit/Write/Edit operations:
...
- **Location**: `.zcode/` directory in project root
- **Naming**: `.zcode/hookify.{descriptive-name}.local.md`
- **Gitignore**: Add `.zcode/*.local.md` to `.gitignore`
