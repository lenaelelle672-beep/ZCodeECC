...
Scripts live at: `~/.zcode/skills/ck/commands/` (expand `~` with `$HOME`).
...
```bash
node "$HOME/.zcode/skills/ck/commands/init.mjs"
```
...
```bash
echo '<confirmed-json>' | node "$HOME/.zcode/skills/ck/commands/save.mjs" --init
```
...
```bash
echo '<json>' | node "$HOME/.zcode/skills/ck/commands/save.mjs"
```
...
```bash
node "$HOME/.zcode/skills/ck/commands/resume.mjs" [arg]
```
...
```bash
node "$HOME/.zcode/skills/ck/commands/info.mjs" [arg]
```
...
```bash
node "$HOME/.zcode/skills/ck/commands/list.mjs"
```
...
If yes:
```bash
node "$HOME/.zcode/skills/ck/commands/forget.mjs" [name]
```
...
```bash
node "$HOME/.zcode/skills/ck/commands/migrate.mjs"
```
For a dry run first:
```bash
node "$HOME/.zcode/skills/ck/commands/migrate.mjs" --dry-run
```
...
The hook at `~/.zcode/skills/ck/hooks/session-start.mjs` must be registered in
`~/.zcode/settings.json` to auto-load project context on session start:

```json
{
  "hooks": {
    "SessionStart": [
      { "hooks": [{ "type": "command", "command": "node \"~/.zcode/skills/ck/hooks/session-start.mjs\"" }] }
    ]
  }
}
```
