...
Use ZCode's built-in memory system enhanced with MCP memory server for structured data.

**Built-in memory** (`~/.zcode/projects/*/memory/`):
...
Write to ~/.zcode/projects/*/memory/ for cross-session recall
...
Use ZCode's scheduled tasks to create recurring agent operations.
...
Trigger ZCode agents remotely for event-driven workflows.
...
Write task queue to ~/.zcode/projects/*/memory/task-queue.md
...
|------------------|---------------|-----|
| Gateway/Router | ZCode dispatch + crons | Scheduled tasks trigger agent sessions |
...
Ensure these are in `~/.zcode.json`:
