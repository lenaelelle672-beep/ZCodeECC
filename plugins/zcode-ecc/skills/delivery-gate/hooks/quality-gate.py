...
Install: cp this file to ~/.zcode/scripts/quality-gate.py
...
def get_project_memory_dir() -> Optional[str]:
    ...
    cwd = os.environ.get('ZCODE_PROJECT_DIR', os.getcwd())
    safe = cwd.replace(':', '-').replace('\\', '-').replace('/', '-')
    mem = os.path.expanduser(f'~/.zcode/projects/{safe}/memory')
...
def main() -> None:
    ...
    # ZCode reads stderr as the hook's response message.
