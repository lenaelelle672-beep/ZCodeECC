...
```json
{
  "hooks": {
    "PreToolUse": [
      {
        "matcher": "Edit",
        "hooks": [{ "type": "command", "command": "node ~/.zcode/scripts/hooks/suggest-compact.js" }]
      },
      {
        "matcher": "Write",
        "hooks": [{ "type": "command", "command": "node ~/.zcode/scripts/hooks/suggest-compact.js" }]
...
- `CLAUDE_CODE_AUTO_COMPACT_WINDOW` — ZCode's native window-size override, in tokens; honored as a fallback when `ECC_CONTEXT_WINDOW_TOKENS` is unset.
...
| Memory files (`~/.zcode/memory/`) | Multi-step conversation context |
...
- Same rules in both `~/.zcode/rules/` and project `.zcode/rules/`
