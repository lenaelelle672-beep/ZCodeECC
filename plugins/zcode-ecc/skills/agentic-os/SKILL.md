---
name: agentic-os
description: "Build persistent multi-agent operating systems on ZCode. Covers kernel architecture, specialist agents, slash commands, file-based memory, scheduled automation, and state management without external databases."
---

# Agentic OS

Treat ZCode as a persistent runtime / operating system rather than a chat session. This skill codifies the architecture used by production agentic setups: a kernel config that routes tasks to specialist agents, persistent file-based memory, scheduled automation, and a JSON/markdown data layer.

## When to Activate

- Building a multi-agent workflow inside ZCode
- Setting up persistent ZCode automation that survives session restarts
...
├── .zcode/commands/  # Slash commands: user-facing CLI
...
| Commands (`.zcode/commands/`) | User-facing slash commands (`/daily-sync`, `/outreach`) | Git-tracked |
...
- Test runner access
- MCP servers as configured in `.zcode/mcp.json`
...
For parallel execution, use ZCode's background task capability or shell scripts that invoke ZCode with specific agent contexts.

## Commands and Daily Workflows

Slash commands are markdown files in `.zcode/commands/`. They define reusable workflows.
...
Place command files in `.zcode/commands/<command-name>.md`. ZCode auto-discovers them. Users invoke them with `/<command-name>`.
...
Agentic OS tasks run on a schedule using external cron, not ZCode's built-in cron (which dies when the session ends).
...
Starting fresh every time ZCode opens.
```
...
- [ ] Scheduled tasks use external cron (LaunchAgent, systemd, pm2), not ZCode's session cron
