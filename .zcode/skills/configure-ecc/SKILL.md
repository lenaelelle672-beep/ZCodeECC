---
name: configure-ecc
description: "Guide ECC installation, update, or reconfiguration from inside ZCode, Codex, or Kimi while respecting each harness's real plugin, scope, and hook capabilities."
---

# Configure Everything ZCode
...
- In ZCode, use the full scope-and-hook wizard below.
...
## ZCode: run the full conversational wizard
...
```bash
node "$ZCODE_PLUGIN_ROOT/scripts/setup.js" --mode claude-plugin \
  --scope <scope> --hooks <hooks> [--move-scope] --dry-run --json
```

If `$ZCODE_PLUGIN_ROOT` is unavailable, use the published npm package:
...
scope. When `$ZCODE_PLUGIN_ROOT` is available, pass the successful setup
...
```bash
node -e 'const { renderTerminalWelcome } = require(process.env.ZCODE_PLUGIN_ROOT + "/scripts/lib/terminal-welcome"); process.stdout.write(renderTerminalWelcome({ action: process.argv[1], version: process.argv[2], color: process.stdout.isTTY }));' "<action>" "<installed-version>"
```
