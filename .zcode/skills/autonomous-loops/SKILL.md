...
Patterns, architectures, and reference implementations for running ZCode autonomously in loops. Covers everything from simple `claude -p` pipelines to full RFC-driven multi-agent DAG orchestration.
...
The `claude -p` flag runs ZCode non-interactively with a prompt, exits when done. Chain calls to build a pipeline:
...
echo "Focus areas: auth module, API rate limiting" > .zcode-context.md
claude -p "Read .zcode-context.md for priorities. Work through them in order."
rm .zcode-context.md
```
...
1. Loads conversation history from `~/.zcode/claw/{session}.md`
...
### Implementation via ZCode Commands

Create `.zcode/commands/infinite.md`:
...
PHASE 4: Deploy sub-agents in parallel (Agent tool). Each receives:
  - Full spec text
...
**A production-grade shell script** that runs ZCode in a continuous loop, creating PRs, waiting for CI, and merging automatically. Created by AnandChowdhary (credit: @AnandChowdhary).
