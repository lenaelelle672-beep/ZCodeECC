...
| Path | Description |
|------|-------------|
| `~/.zcode/skills/` | Global skills (all projects) |
| `{cwd}/.zcode/skills/` | Project-level skills (if the directory exists) |
...
If the project has no `.zcode/skills/` directory, only global skills and commands are evaluated.
...
**Results cache:** `~/.zcode/skills/skill-stocktake/results.json`

## Quick Scan Flow

Re-evaluate only skills that have changed since the last run (5–10 min).

1. Read `~/.zcode/skills/skill-stocktake/results.json`
2. Run: `bash ~/.zcode/skills/skill-stocktake/scripts/quick-diff.sh \
         ~/.zcode/skills/skill-stocktake/results.json`
   (Project dir is auto-detected from `$PWD/.zcode/skills`; pass it explicitly only if needed)
3. If output is `[]`: report "No changes since last run." and stop
4. Re-evaluate only those changed files using the same Phase 2 criteria
5. Carry forward unchanged skills from previous results
6. Output only the diff
7. Run: `bash ~/.zcode/skills/skill-stocktake/scripts/save-results.sh \
         ~/.zcode/skills/skill-stocktake/results.json <<< "$EVAL_RESULTS"`

## Full Stocktake Flow

### Phase 1 — Inventory

Run: `bash ~/.zcode/skills/skill-stocktake/scripts/scan.sh`
...
```
Scanning:
  ✓ ~/.zcode/skills/         (17 files)
  ✗ {cwd}/.zcode/skills/    (not found — global skills only)
```
