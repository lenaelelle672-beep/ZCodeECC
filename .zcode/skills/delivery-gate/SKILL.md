...
ZCode's built-in checks cover code quality (build → type → lint → test). But there's a different failure mode: the agent produces working code while the **session hygiene was neglected** — learning not captured, rationalized shortcuts, disk running out silently.
...
```bash
cp quality-gate.py ~/.zcode/scripts/
```
...
```json
{
  "hooks": {
    "Stop": [{
      "hooks": [{
        "type": "command",
        "command": "python3 ~/.zcode/scripts/quality-gate.py",
        "timeout": 5000
      }]
    }]
  }
