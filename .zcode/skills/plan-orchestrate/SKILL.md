...
Let `<claude-home>` denote the ZCode home directory: `~/.zcode` on macOS/Linux, `%USERPROFILE%\.zcode` on Windows. Resolve it the way the host platform resolves the user home directory (do not hardcode `~`).
