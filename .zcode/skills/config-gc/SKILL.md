---
name: config-gc
description: "Garbage collection for your ZCode configuration. Periodically scans ~/.zcode (skills, memory, hooks, permissions, MCP servers, caches) for redundant, stale, orphaned, or low-value items, then walks the user through a confirm-each-deletion cleanup. Use when the user says \"clean up my config\", \"config GC\", \"too many skills\", \"audit my setup\", \"my .zcode is bloated\", or asks for a periodic config review."
---

# Config GC — Garbage Collection for ZCode Setups
...
- The user asks to clean up, audit, or slim down their ZCode configuration
...
Do NOT activate for: cleaning project source code (that's refactoring), clearing chat history, or uninstalling ZCode itself.
...
4. **Soft-delete first.** Rename to `.disabled` > move to `~/.zcode/_gc_trash/` > real deletion. Always keep an undo path.
5. **Forced human-in-the-loop.** Every candidate gets its own `[y/n/skip]` confirmation. No "yes to all" shortcut.
6. **Keep a log.** Every GC run appends to `~/.zcode/gc_log.md`: what was touched, why, and how to undo it.

## Scan Channels

| # | Channel | Path | Staleness / redundancy signals |
|---|---------|------|--------------------------------|
| 1 | Skills | `~/.zcode/skills/*/` | Heavily overlapping names; never triggered in recent transcripts; domain mismatch with the user's actual work; broken or empty SKILL.md |
| 2 | Memory | `~/.zcode/**/memory/*.md` + its index | Multiple index entries for one topic; contents contradicting newer entries; dates that have passed; orphan files missing from the index; sub-100-word fragments that should merge |
| 3 | Hooks | `~/.zcode/hooks/` + settings | Scripts present on disk but referenced by no hook config; old versions superseded by rewrites |
...
| 5 | MCP servers | `~/.zcode.json` or project `.mcp.json` | Servers that fail to connect; functional duplicates; long-unused |
...
| 7 | Project history | `~/.zcode/projects/*/` | Stale handoff snapshots; session records superseded by newer state |
...
5. **Log** the run to `~/.zcode/gc_log.md`: timestamp, items actioned, undo instructions.
...
```bash
for f in ~/.zcode/hooks/*; do
  name=$(basename "$f")
  grep -rq "$name" ~/.zcode/settings.json ~/.zcode/settings.local.json 2>/dev/null \
    || echo "ORPHAN: $f"
done
```
...
```bash
jq -r '.permissions.allow[]' ~/.zcode/settings.local.json | sort | uniq -d
if jq -e '.permissions.allow | index("Bash(*)")' ~/.zcode/settings.local.json >/dev/null; then
  jq -r '.permissions.allow[]' ~/.zcode/settings.local.json \
...
```bash
find ~/.zcode/file-history ~/.zcode/shell-snapshots -type f -mtime +30 \
...
```bash
gc_date=$(date +%Y-%m-%d)
mkdir -p ~/.zcode/_gc_trash/$gc_date
mv ~/.zcode/skills/dead-skill ~/.zcode/_gc_trash/$gc_date/
echo "$(date -Iseconds) moved skills/dead-skill -> _gc_trash/$gc_date/ (undo: mv back)" >> ~/.zcode/gc_log.md
```

Removing a confirmed-redundant permission entry (JSON has no comments — back up, log, then edit):

```bash
cp ~/.zcode/settings.local.json ~/.zcode/settings.local.json.bak
echo "$(date -Iseconds) removed permission entry: Bash(git push) (undo: restore from .bak or re-add)" >> ~/.zcode/gc_log.md
jq '.permissions.allow -= ["Bash(git push)"]' ~/.zcode/settings.local.json.bak \
...
- **Touching anything outside `~/.zcode`** (or the project's `.zcode/`). Config GC never wanders into source trees.
