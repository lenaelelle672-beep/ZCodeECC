...
- You just wrote a plan artifact (`.zcode/plans/*.plan.md` from `/plan`) and
...
(ZCode, Codex, Cursor, Gemini, OpenCode, Copilot). Trigger it however your
harness surfaces skills — e.g. `/plan-canvas` in ZCode, `$plan-canvas` in
...
- The server is loopback-only and exits after 30 idle minutes
  ...
  in `~/.zcode/plan-canvas/` (`ECC_PLAN_CANVAS_STATE_DIR`).
