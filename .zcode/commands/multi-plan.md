...
> **Prerequisite:** Requires the external `ccg-workflow` runtime, which is **not** part of the base ECC install. Initialize it with `npx ccg-workflow` to provision `~/.zcode/bin/codeagent-wrapper` and the `~/.zcode/.ccg/prompts/*` role files this command depends on. Without that runtime, this command will not run correctly.
...
- **Planning Only**: This command allows reading context and writing to `.zcode/plan/*` plan files, but **NEVER modify production code**
...
```
Bash({
  command: "~/.zcode/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend <codex|gemini> {{GEMINI_MODEL_FLAG}}- \"$PWD\" <<'EOF'
...
| Phase | Codex | Gemini |
|-------|-------|--------|
| Analysis | `~/.zcode/.ccg/prompts/codex/analyzer.md` | `~/.zcode/.ccg/prompts/gemini/analyzer.md` |
| Planning | `~/.zcode/.ccg/prompts/codex/architect.md` | `~/.zcode/.ccg/prompts/gemini/architect.md` |
...
**If ace-tool MCP is NOT available**, use ZCode built-in tools as fallback:
...
1. **Codex Backend Analysis**:
   - ROLE_FILE: `~/.zcode/.ccg/prompts/codex/analyzer.md`
...
2. **Gemini Frontend Analysis**:
   - ROLE_FILE: `~/.zcode/.ccg/prompts/gemini/analyzer.md`
...
1. **Codex Plan Draft** (Backend authority):
   - ROLE_FILE: `~/.zcode/.ccg/prompts/codex/architect.md`
...
2. **Gemini Plan Draft** (Frontend authority):
   - ROLE_FILE: `~/.zcode/.ccg/prompts/gemini/architect.md`
...
2. Save plan to `.zcode/plan/<feature-name>.md` (extract feature name from requirement, e.g., `user-auth`, `payment-module`)
3. Output prompt in **bold text** (MUST use actual saved file path):

---
**Plan generated and saved to `.zcode/plan/actual-feature-name.md`**
...
```
/ccg:execute .zcode/plan/actual-feature-name.md
```
---
...
- **First planning**: `.zcode/plan/<feature-name>.md`
- **Iteration versions**: `.zcode/plan/<feature-name>-v2.md`, `.zcode/plan/<feature-name>-v3.md`...
...
2. Update `.zcode/plan/<feature-name>.md` file
...
```bash
/ccg:execute .zcode/plan/<feature-name>.md
```
