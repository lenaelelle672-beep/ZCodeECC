...
```bash
mkdir -p .zcode/PRPs/reports
```

Write report to `.zcode/PRPs/reports/{plan-name}-report.md`:
...
```bash
mkdir -p .zcode/PRPs/plans/completed
mv "$ARGUMENTS" .zcode/PRPs/plans/completed/
```
...
### Artifacts
- Report: `.zcode/PRPs/reports/{name}-report.md`
- Archived Plan: `.zcode/PRPs/plans/completed/{name}.plan.md`
