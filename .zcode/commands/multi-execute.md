...
Bash({
  command: "~/.zcode/bin/codeagent-wrapper {{LITE_MODE_FLAG}}--backend <codex|gemini> {{GEMINI_MODEL_FLAG}}resume <SESSION_ID> - \"$PWD\" <<'EOF'
...
| Phase | Codex | Gemini |
|-------|-------|--------|
| Implementation | `~/.zcode/.ccg/prompts/codex/architect.md` | `~/.zcode/.ccg/prompts/gemini/frontend.md` |
| Review | `~/.zcode/.ccg/prompts/codex/reviewer.md` | `~/.zcode/.ccg/prompts/gemini/reviewer.md` |
...
1. **Identify Input Type**:
   - Plan file path (e.g., `.zcode/plan/xxx.md`)
...
1. Call Gemini (use `~/.zcode/.ccg/prompts/gemini/frontend.md`)
...
1. Call Codex (use `~/.zcode/.ccg/prompts/codex/architect.md`)
2. Input: Plan content + retrieved context + target files
3. OUTPUT: `Unified Diff Patch ONLY. Strictly prohibit any actual modifications.`
...
1. **Codex Review** (`run_in_background: true`):
   - ROLE_FILE: `~/.zcode/.ccg/prompts/codex/reviewer.md`
...
2. **Gemini Review** (`run_in_background: true`):
   - ROLE_FILE: `~/.zcode/.ccg/prompts/gemini/reviewer.md`
   - Input: Changed Diff + target files
